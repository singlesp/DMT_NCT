...
transitions = transitions - 2; %first tr skipped
...
for i = 1:nsub
    ...
    [global_CE_dmt_pre(i,:),regional_CE_dmt_pre(i,:,:)] = time_resolved_control_energy(Anorm,T,B,ts);

end

save(fullfile([basedir,'results/regional_mins8_CE_DMT',note,'.mat']),'global_CE_dmt','regional_CE_dmt','global_CE_dmt_pre','regional_CE_dmt_pre');
...
for i = 1:nsub
    ...
    [global_CE_pcb_pre(i,:),regional_CE_pcb_pre(i,:,:)] = time_resolved_control_energy(Anorm,T,B,ts);


end

save(fullfile([basedir,'results/regional_mins8_CE_PCB',note,'.mat']),'global_CE_pcb','regional_CE_pcb','global_CE_pcb_pre','regional_CE_pcb_pre');


%% SI FIGURE 4, left

load(fullfile([basedir,'results/regional_mins8_CE_DMT',note,'.mat']),'global_CE_dmt','regional_CE_dmt','global_CE_dmt_pre','regional_CE_dmt_pre');
...
input_filename = 'results/dominance/diffE';
writetable(mydata, [basedir,input_filename,note, '.csv']);

%call from terminal:
% python3 ~/Documents/GIT/DMT_NCT/fxns/dominance_analysis_al857.py ~/Documents/GIT/DMT_NCT/results/dominance/diffE_gsr_volnorm.csv  target 5 0 ~/Documents/GIT/DMT_NCT/results/dominance/diffE_gsr_volnorm
